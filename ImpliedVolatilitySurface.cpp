...
// Initialization List !!!
ImpliedVolatilitySurface::ImpliedVolatilitySurface(const Vector& maturities, 
	const Vector& strikes, const Matrix& implied_vols, const double& risk_free_rate)
	: _maturities(maturities), _strikes(strikes), 
	_market_implied_vols(implied_vols), _risk_free_rate(risk_free_rate)
{
...
ImpliedVolatilitySurface::ImpliedVolatilitySurface(
	...
	: _maturities(implied_vol_surface._maturities), _strikes(implied_vol_surface._strikes),
	_market_implied_vols(implied_vol_surface._market_implied_vols),
	_risk_free_rate(implied_vol_surface._risk_free_rate)
{
	if (!check_ordered_vectors())
		throw "The maturities and strikes must be ordered and positive [strictly positive for maturities]";
	if (!check_dimensions())
